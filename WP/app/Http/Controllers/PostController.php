...
class PostController extends Controller
{
    ...
    return redirect()->route('matching.show', ['cat' => $request->cat_id, 'user' => auth()->id()]); // 適切なルートにリダイレクト
    }

    /**
     * Display the specified resource.
     */
    public function show($cat, $user, Post $post)
    {
        // cat_idとuser_idを使って、正確なmatchingを取得
        $matching = Matching::where('cat_id', $cat)->where('user_id', $user)->first();
