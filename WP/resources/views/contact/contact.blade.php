...
<section class="mt-20 pb-24">
  <div class="w-192 mx-auto">
    <p class="text-left">以下のフォームに必要事項をご入力の上、ご送信下さい。<br>
      ...
      なお、info@withpets.ne.jpからの返信が受信できるように事前に設定のご確認をお願い致します。
    </p>
    <div class="mt-8">
      <!-- ▼▼▼▼エラーメッセージ▼▼▼▼　-->
      @if ($errors->any())
      ...
      <form action="{{ route('contact') }}" method="POST">
        @csrf
        <div class="mb-4">
          <label for="gname" class="block text-left p-1 my-1 font-medium">動物保護団体名<span class="text-white text-xs bg-yellow-400 mx-2 py-1 px-2">必須</span></label>
          <input id="gname" name="gname" class="w-full p-4 text-xs leading-none bg-blueGray-50 rounded outline-none border" type="text" placeholder="例）NPO法人 ペットとシニアを救う With Pets" value={{ old('gname') }}>
          @if($errors->has('gname'))
            <p class="text-pink-400 text-xs mt-2">{{ $errors->first('gname') }}</p>
          @endif
        </div>
        <div class="mb-4">
